...
//		currentPath = "C:\\tools\\Set-Schedule";
	...
	private static void setSchedules(Workbook wb, List<Schedule> schedules, boolean logger) {
		...
		for (; idx < lastRowNum; idx++) {
			...
			if (understandRate != null && understandRate.compareTo(PERCENT100) == 0){
			...
			if (utdRate != null && utdRate.compareTo(PERCENT100) == 0) {
				utdReviewRate = schedule.getUtdReviewRate();
			...
			if (utdReviewRate != null && utdReviewRate.compareTo(PERCENT100) == 0){
			...
			/** When the utdReviewRateCell is right.**/
			if (codingRate != null && codingRate.compareTo(PERCENT100) == 0) {
			...
			/** When the utdReviewRateCell is right.**/
			if (cdiRate != null && cdiRate.compareTo(PERCENT100) == 0) {
